...
class SpecificDegreeScreen extends React.Component {
  ...
  _loadClient = async () => {
    ...
    try {
        const value = await AsyncStorage.getItem('SpecificDegree');
        if (value !== null) {
            const res = await this.FetchService.getDegreeSpecific(value);
            if (res === false) {
              Alert.alert(
  ...
  backButtonHandler = () => {
    this.props.navigation.navigate("Degree");
    return true;
  }
  ...
  _buttonMethod = async (item) => {
    await AsyncStorage.setItem('SpecificDegreeDetail', JSON.stringify(item)).then(() => {
